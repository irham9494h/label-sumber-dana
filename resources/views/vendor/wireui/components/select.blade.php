<div {{ $attributes->only(['class', 'wire:key'])->class('relative w-full') }}
    ...
    <div class="relative">
        @if ($label)
        <x-dynamic-component :component="WireUi::component('label')" class="mb-1" :label="$label"
            :has-error="$name && $errors->has($name)" :disabled="$disabled" x-on:click="toggle"
            ...
            <x-slot name="prepend">
                <div :class="{
                    ...
                    <template x-if="!config.multiselect">
                        <div @class([ 'absolute left-0 inset-y-0 w-[calc(100%-3.5rem)] flex items-center' , 'pl-2.5'=>
                            $icon,
                            'pl-3.5' => !$icon,
                            ])
                            x-on:click="toggle">
            ...
            <x-slot name="append">
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 gap-x-2">
                    ...
                    <button tabindex="-1" x-on:click="toggle" type="button">
                        <x-dynamic-component :component="WireUi::component('icon')" class="w-5 h-5
    ...
    <x-wireui::parts.popover :margin="(bool) $label" class="sm:max-w-full">
        <template x-if="asyncData.api || (config.searchable && options.length >= @toJs($minItemsForSearch))">
            <div class="px-2 my-2" wire:key="search.options.{{ $name }}">
                <x-dynamic-component :component="WireUi::component('input')"
                    :placeholder="trans('wireui::messages.searchHere')" class="bg-slate-100" x-ref="search"
